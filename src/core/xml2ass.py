...
def convert_xml_to_ass(xml_path: str, ass_path: str, settings: dict = None):
    """
    将Niconico XML弹幕文件转换为ASS字幕文件
    
    参数:
        xml_path: 输入的XML文件路径
        ass_path: 输出的ASS文件路径
        settings: 配置字典，可包含以下键：
            - limit_line_amount: 弹幕行数限制（默认：11）
            - ass_code: ASS代码前缀（默认：\\fnMS PGothic\\b1\\bord2\\blur0）
            - danmaku_size: 弹幕大小（默认：46）
            - danmaku_density: 弹幕密度（默认：10）
            - start_time_adjust: 时间偏移秒数（默认：0）
            - danmaku_speed_adjust: 速度调整秒数（默认：1）
            - use_ass_colors: 是否使用弹幕颜色（默认：True）
            - filter_outsider: 是否过滤非会员弹幕（默认：False）
            - use_speed_algo: 是否使用速度算法（默认：False）
            - manual_add_office_id: 手动添加运营ID列表（默认：[]）
            - filter_keywords: 屏蔽关键词列表（默认：[]）
    """
    if settings is None:
        settings = {}
    ...
    # 从settings构建Config
    config = Config(
        limit_line_amount=get_int('limit_line_amount', 11),
        ass_code=settings.get('ass_code') or r"\fnMS PGothic\b1\bord2\blur0",
        danmaku_size=get_int('danmaku_size', 46),
        danmaku_density=get_int('danmaku_density', 10),
        start_time_adjust=get_float('start_time_adjust', 0.0),
        danmaku_speed_adjust=get_float('danmaku_speed_adjust', 1.0),
        use_ass_colors=get_bool('use_ass_colors', True),
        difficult_vip=get_bool('difficult_vip', False),
        filter_outsider=get_bool('filter_outsider', False),
        use_speed_a=get_bool('use_speed_algo', False),
        manual_add_office_id=get_list('manual_add_office_id', []),
        filter_keywords=get_list('filter_keywords', [])
    )
    
    # 转换
    converter = NicoXML2ASS(config)
    converter.load_xml(xml_path)
    converter.save_ass(ass_path)
...
def main():
    ...
    settings = {
        ...
        'use_speed_algo': args.use_speed_algo,
    ...
    # 转换
    convert_xml_to_ass(args.input, output_path, settings)
